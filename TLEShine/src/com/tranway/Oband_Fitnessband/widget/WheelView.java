...
public class WheelView extends View {
	...
	/** Center line color */
	private static final int LINE_CENTER_COLOR = 0xFFDDDDDD;

	/** Top and bottom shadows colors */
	private static final int[] SHADOWS_COLORS = new int[] { 0xFFFFFFFF, 0x55FFFFFF, 0x00FFFFFF };
	...
	public void setCurrentItem(int index, boolean animated) {
		...
		if (index < 0 || index >= adapter.getItemsCount()) {
			Log.e("WheelView", "set (item=" + index + ") > (max=" + adapter.getItemsCount());
			if (isCyclic) {
				while (index < 0) {
	...
	private void initResourcesIfNecessary() {
		...
		if (centerLinePaint == null) {
			...
			centerLinePaint.setColor(LINE_CENTER_COLOR);
		}
	...
	private int getDesiredHeight(Layout layout) {
		...
		int itemOffset = (int) context.getResources().getDimension(VALUE_TEXT_SIZE);
		int desired = getItemHeight() * visibleItems - itemOffset * 2 - ADDITIONAL_ITEM_HEIGHT;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsLayout == null) {
			if (itemsWidth == 0) {
				// calculateLayoutWidth(getMeasuredWidth(),
				// MeasureSpec.EXACTLY);
	...
	private void drawCenterLine(Canvas canvas) {
		...
		centerLinePaint.setStrokeWidth(2.0f);
		canvas.drawLine(0, center - offset, getMeasuredWidth(), center - offset, centerLinePaint);
		canvas.drawLine(0, center + offset, getMeasuredWidth(), center + offset, centerLinePaint);
	}
